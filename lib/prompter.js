...
console.log(subStringsArray);
            ...
            */
            function compareString (results) {
                // if(scriptText == '') {
                ...
                transcriberWord = cleanText(getTranscrriberWord(results));
                ...
                if (jump) {
                    jumpPromopterToSentence(transcriberString);
                }
                ...
                while (currentIndex < windowEndIndex
                    ...
                    if(algo == 'LevenshteinDistance') {
                        if(calculateLevenshteinDistance(scriptCurrentWord, transcriberWord) <= 1) {                                
                            movePointers(windowCurrentIndex);
                            break;
                        }
            ...
            function jumpPromopterToSentence(transcriberString) {
                ...
                while(nextStringNumber < stringWindowEndIndex
                    && subStringsArray.length >= nextStringNumber
                    && subStringsArray.length >= stringWindowEndIndex) {
                    console.log('subStringsArray : ', subStringsArray[nextStringNumber]);
                    var newDistance = calculateJaroDistance(subStringsArray[nextStringNumber], transcriberSubString);
                    console.log(subStringsArray[nextStringNumber], transcriberSubString, newDistance);
                    if(newDistance > 0.6) {
                         if(newDistance > distance) {
                             console.log('newDistance : ', newDistance, nextStringNumber);
                             distance = newDistance;
                             scriptStringIndex = nextStringNumber;
                             var stringWords = document.querySelectorAll('[stringindex="'+scriptStringIndex+'"]');
                             stringWordIndex = stringWords[stringWords.length-1].getAttribute('wordIndex');
                             stringWordId = stringWords[stringWords.length-1].getAttribute('id');
                             console.log(stringWordId);
                             // stringWords.forEach((wordElement, index) => {
                             // })
                         }
                    }
                    nextStringNumber++;
                }
                // subStringsArray.forEach((scriptString, strIndex) => {
                // });
                ...
                console.log('jumpppppppppp to: ', scriptStringIndex,stringWordId, stringWordIndex);
            }
            ...
            function movePointers(windowCurrentIndex) {
