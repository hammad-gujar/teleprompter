...
function initPointers() {    
    ...
    pauseCorrectionLimit = 3;
    pauseCorrectionCount = 0;
    pauseIndexat = 0;
    correctionCountClearTimeIsOn = false;
}

function transcriberKeysSaved() {
    if (!accessKey || !secretKey ) {
        showNotification('error', 'Error', 'Please enter the transriber keys in configurations', false);
        return false;
    }
    return true;
}
...
function loadSavedPrefrences() {
    ...
    if (configs !== null) {
        configs.forEach((config, index) => {
            ...
            } else if (config.type === 'secretKey') {
                secretKey = config.value;
                document.getElementById('secretKey').value = secretKey;             
            } else if (config.type === 'accessKey') {
                accessKey = config.value;
...
function SaveAccessKey() {
    ...
    saveToLocalStorage('accessKey', accessKey);
}

function SaveSecretKey() {
    secretKey = document.getElementById('secretKey').value;
    saveToLocalStorage('secretKey', secretKey);
}

function clearKeys() {
    accessKey = document.getElementById('accessKey').value = "";
    secretKey = document.getElementById('secretKey').value = "";
    saveToLocalStorage('accessKey', '');
    saveToLocalStorage('secretKey', '');
}
        ...
        if (algo == 'LevenshteinDistance') {
            ...
            if (calculateJaroDistance(scriptCurrentWord, transcriberWord) > 0.9) {
                ...
                if(!pause && active) {
                    ...
                    if (!correctionCountClearTimeIsOn && pause || !correctionCountClearTimeIsOn && !active) {
